...
%   3. Determine a global cubic region of interest, ROI, from the combined
%      coordinate bounds of all input CSV files.
...
%
% Inputs:
%   - data_export_emax_plus_full_set9_2.csv
%   - data_export_emax_minus_full_set9_2.csv
%
...
%
% Note:
%   Eqv_strain is assumed to be a nonnegative equivalent strain measure from
%   each loading state. The average of the positive- and negative-state hotspot
%   percentiles is used here as an approximate hotspot strain amplitude.
csvFiles = { ...
    'data_export_emax_plus_full_set9_2.csv', ...
    'data_export_emax_minus_full_set9_2.csv'};
...
%% ---------------------- DETERMINE GLOBAL CUBIC ROI -----------------------
% Determine a single cubic region of interest, ROI, from the combined coordinate
% bounds of all input CSV files.
%
% The script finds the global minimum and maximum coordinate values over all
% files. These bounds are then used to define a cube centered at the global
% bounding-box center with side length equal to the largest bounding-box extent.
%
% This creates one consistent ROI definition that is applied to every file.
% Note that this is based on the union of the input coordinate bounds, not the
% overlapping/intersection region shared by all files.
...
for f = 1:nFiles
    ...
    % Apply the global cubic ROI mask.
    %
...
%   2. A single experimental fatigue data point from DARPA Set-9 KH.
...
%
% DARPA Set-9 KH:
%   Nf_set9_exp : measured cycles to failure
%   A_set9_exp  : applied nominal strain amplitude, mm/mm
Nf_set9_exp       = [25410];
A_set9_exp        = [0.003];
...
error_bar_cycles  = error_bar_percent * Nf_set9_exp;
...
%
% eps_nom_target:
%   Target nominal strain amplitude, here taken from the experimental Set-9
%   loading amplitude.
%
% eps_nom_ref:
...
% Estimate the fatigue life for the reference CPFE case.
%
% The local hotspot strain amplitude, eps_a_hot_ref, is interpreted through the
% LPBF baseline CMB curve to obtain the life associated with that local strain
% demand. For plotting on the nominal strain-life diagram, the predicted life is
% shown at the corresponding nominal reference strain amplitude, eps_nom_ref.
%
% Nf_pred_ref:
...
%   3. Experimental Set-9 KH fatigue data point
