...
% -------------------------------------------------------------------------
% This script generates a 3D voxel/grid-based synthetic polycrystalline
% microstructure around an imported single-pore XCT region. Grain IDs are
% assigned using randomly seeded anisotropic grains with lognormally
% distributed grain diameters and aspect ratios.
%
% The imported pore mask is used to compute the pore volume, centroid, and
% principal directions. It is then replaced by an equivalent ellipsoidal pore
% with the same volume and aspect ratios inferred from the second central
% moments. This equivalent ellipsoidal pore mask overwrites the generated grain
% phase map.
%
...
%   5. Replace the imported pore by an equivalent ellipsoidal pore and overwrite
%      those sites with the pore/void phase ID.
...
Ly_um = pore_export_data.metadata.domain_size_um(1);
Lx_um = pore_export_data.metadata.domain_size_um(2);
Lz_um = pore_export_data.metadata.domain_size_um(3);
...
% --- Replace imported XCT pore with equivalent ellipsoid mask ---
% The ellipsoid is centered at the pore centroid, aligned with the pore
% covariance principal directions, assigned covariance-based axis ratios, and
% scaled so its continuous volume matches the imported pore volume.

% Extract pore points as an N x 3 list (mm)
xp = X(pore_mask); 
yp = Y(pore_mask); 
zp = Z(pore_mask);
P  = double([xp(:), yp(:), zp(:)]);

% Pore volume estimated from the number of pore voxels/grid cells.
% Assumes each true pore-mask entry represents volume dx^3.
Nv = size(P,1);
V  = Nv * dx^3;
...
% Principal directions from the pore-point covariance matrix.
% The eigenvectors define the ellipsoid principal axes, and the eigenvalues
% define the relative squared spreads along those axes.
C = (Q.'*Q) / Nv;
[R, D] = eig(C);
...
% Relative semi-axis lengths from sqrt(eigenvalues).
% For a uniform ellipsoid, covariance eigenvalues are proportional to the
% squared semi-axis lengths, so sqrt(lam) gives the correct axis ratios.
s = sqrt(max(lam, eps));

% Scale the continuous ellipsoid to match the pore volume:
%       (4/3)*pi*a*b*c = V
% Let a = k*s1, b = k*s2, c = k*s3.
% Note: after voxelization, nnz(ellipsoid_mask)*dx^3 may differ slightly from V.
k      = ((3*V) / (4*pi*prod(s)))^(1/3);
a      = k * s(1);
b      = k * s(2);
csemi  = k * s(3);

% Build ellipsoid mask on grid
% Transform every grid point into ellipsoid principal-axis coordinates
Xc = double(X) - centroid(1);
Yc = double(Y) - centroid(2);
Zc = double(Z) - centroid(3);

% Rotate into principal frame: u = R' * [Xc;Yc;Zc]
% u_j = R(:,j)' * [Xc; Yc; Zc]  — projection onto j-th eigenvector
u1 = R(1,1)*Xc + R(2,1)*Yc + R(3,1)*Zc;  % coordinate along axis a
u2 = R(1,2)*Xc + R(2,2)*Yc + R(3,2)*Zc;  % coordinate along axis b
u3 = R(1,3)*Xc + R(2,3)*Yc + R(3,3)*Zc;  % coordinate along axis csemi

% Ellipsoid equation:
%
%       (u1/a)^2 + (u2/b)^2 + (u3/csemi)^2 <= 1
%
% Grid points satisfying this equation are assigned to the equivalent pore.
ellipsoid_mask = (u1.^2/a^2 + u2.^2/b^2 + u3.^2/csemi^2) <= 1;
...
%% 5. Apply Equivalent Ellipsoidal Pore Mask
% -------------------------------------------------------------------------
% Overwrite the synthetic grain assignments with the pore/void phase wherever
% the equivalent ellipsoidal pore mask is true.
%
% Phase convention:
%   solid grains = 1:N_grains
%   pore/void    = N_grains + 1
%
% Note:
%   The imported pore_mask was used to compute pore volume, centroid, and
%   second-moment principal axes. The final pore geometry applied here is
%   ellipsoid_mask.
% -------------------------------------------------------------------------

phase(ellipsoid_mask) = N_grains + 1;
...
fprintf('Grains in limited domain: %d (pore phase in export = %d)\n', ...
