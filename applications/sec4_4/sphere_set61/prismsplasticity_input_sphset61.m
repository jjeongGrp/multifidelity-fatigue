...
%
% The imported pore mask is used to compute the pore volume and centroid, then
% replaced by an equivalent spherical pore with the same volume. This equivalent
% spherical pore mask overwrites the generated grain phase map.
%
...
%   5. Replace the imported pore by an equivalent spherical pore and overwrite
...
% Replace Imported Pore with Equivalent Spherical Pore
% -------------------------------------------------------------------------
% Convert the imported pore mask into an equivalent spherical pore.
%
% Purpose:
%   The imported pore may have an irregular XCT-derived shape. For this
%   controlled single-pore study, the pore is idealized as a sphere with:
%
%     - the same volume as the imported pore mask
%     - the same centroid as the imported pore mask
%
% The final pore assignment later uses sphere_mask rather than pore_mask.
%
% Units:
%   X, Y, Z, dx are in millimeters, so V is in mm^3 and r is in mm.
% -------------------------------------------------------------------------

% Number of pore grid sites in the imported mask.
Nv = nnz(pore_mask);

% Imported pore volume.
V  = Nv * dx^3;

% Imported pore centroid in physical coordinates.
xc = mean(X(pore_mask));
yc = mean(Y(pore_mask));
zc = mean(Z(pore_mask));

% Equivalent sphere radius from V = 4/3*pi*r^3.
r = (3*V/(4*pi))^(1/3);

% Equivalent spherical pore mask on the same grid.
sphere_mask = ((X-xc).^2 + (Y-yc).^2 + (Z-zc).^2) <= r^2;

fprintf('Overwriting grains with equivalent sphere pore...\n');
...
%% 5. Apply Equivalent Spherical Pore Mask
% -------------------------------------------------------------------------
% Overwrite the synthetic grain assignments with the pore/void phase wherever
% the equivalent spherical pore mask is true.
%
% Phase convention:
%   solid grains = 1:N_grains
%   pore/void    = N_grains + 1
%
% Note:
%   The imported pore_mask was used to compute pore volume and centroid.
%   The final pore geometry applied here is sphere_mask.
% -------------------------------------------------------------------------

phase(sphere_mask) = N_grains + 1;
